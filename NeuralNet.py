...
class Perceptron(object):
    ...
    def sigmoidDeriv(self, value):
        ...
        """
        return exp(-value) / (float(1 + exp(-value)) * float(1 + exp(-value)))
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            curr = [percep.sigmoidActivation(curr) for percep in layer]
            outputs.append(curr)
        return outputs
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples: # For each example
            ...
            lastLayerOutput = allLayerOutput[len(allLayerOutput) - 1]
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    delta = gPrime * sum([nextLayer[x].weights[neuronNum + 1] * deltas[0][x] for x in range(len(nextLayer))])
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for input, expected in examplesTest:
        ...
        output = [round(r) for r in output[len(output) - 1]]
        if output == expected:
            testCorrect += 1
        else:
            testError += 1
    testAccuracy = testCorrect / (testError + testCorrect) # Num correct/num total
    
    print 'Feed Forward Test correctly classified %d, incorrectly classified %d, test percent error  %f\n' % (testCorrect, testError, (1 - testAccuracy))
